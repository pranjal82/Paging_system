...
private:
    ...
    // For LRU
    list<int> lruList;  // Store page indices
...
public:
    ...
    void accessPageLRU(int page);  // LRU page access function
    int movePageToPhysicalMemory(int pid, int logicalPage);
    void exitSystem();
...
PagingSystem::PagingSystem(int mainMemSize, int virtMemSize, int pageSize)
    : M(mainMemSize), V(virtMemSize), P(pageSize), pidCounter(1) {
...
int PagingSystem::movePageToPhysicalMemory(int pid, int pageno) {
...
void PagingSystem::runProcess(int pid) {
    ...
    if (processMap.find(pid) == processMap.end()) {
        cout << "Invalid PID given, not in processMap \n";
        return;
    }
    ...
    for(int i=1;i<vec.size();i++){
      ...
      //      iss >> x >> y >> z;
            ...
            int ppn_x=  movePageToPhysicalMemory(pid, x / P);  // Ensure the page is in physical memory
           int ppn_y= movePageToPhysicalMemory(pid, y / P);  // Ensure the page is in physical memory
           int ppn_z= movePageToPhysicalMemory(pid, z / P);  // Ensure the page is in physical memory
            int value_x = memory[ppn_x * P + x % P];
            int value_y = memory[ppn_y * P + y % P];
            int result= value_x + value_y;
              memory[ppn_z * P + z % P] = result;
              cerr<<"MEMORY INDEX = "<<ppn_x * P + x % P<<" "<<ppn_y * P + y % P<<endl;
       ...
       //     iss >> x;
          ...
          int ppn_x=  movePageToPhysicalMemory(pid, x / P);
           ...
           // iss >> a >> y;
            ...
            //cout<<"Hi1\n";
            int ppn_y = movePageToPhysicalMemory(pid, y / P);
           // cout<<"Hi2\n";
...
// LRU: Simulate access to a page
// void PagingSystem::accessPageLRU(int page) {
//     if (lruMap.find(page) != lruMap.end()) {
//         lruList.erase(lruMap[page]);  // Remove old position
//     }
//     lruList.push_front(page);         // Add to front (most recently used)
//     lruMap[page] = lruList.begin();   // Update map with new position

//     // If memory is full, call replacement algorithm
//     if (lruList.size() > physicalMemory.size()) {
//         replacePageLRU(page);
//         //replaceFIFO(page);
//     }
// }
...
// void PagingSystem::replacePageLRU(int newPage) {

//     if (physicalMemory.find(newPage) != physicalMemory.end()) {
//         // If the page is already in memory, move it to the front of the LRU list
//         lruList.remove(newPage);
//         lruList.push_front(newPage);
//         return;
//     }

//     int lruPage = lruList.back();  // Least recently used page is at the back
//     lruList.pop_back();
//     lruMap.erase(lruPage);

//     cout << "LRU Replacement: Replacing page " << lruPage << " with new page " << newPage << "\n";
//     // Update physical memory mapping here
//     physicalMemory.erase(lruPage);

//         // You can optionally clear any page table entries or metadata for the evicted page
//         for (auto& processEntry : processMap) {
//             for (auto& pageTableEntry : processEntry.second.pageTable) {
//                 if (pageTableEntry.physicalPage == lruPage) {
//                     pageTableEntry.physicalPage = -1;  // Mark as not in physical memory
//                     pageTableEntry.isInMemory = false;  // Mark as swapped out
//                 }
//             }
//         }
//         lruList.push_front(newPage);  // Add the new page to the front (most recently used)
//     physicalMemory[newPage] = 0;  // Initialize the new page in physical memory
// }

// FIFO replacement algorithm
// void PagingSystem::replacePageFIFO(int newPage) {
//     if(fifoQueue.empty()){
//         cerr<<"queue is empty\n";
//         return;
//     }
...
//     fifoQueue.pop();
//     cout << "FIFO Replacement: Replacing page " << oldestPage << " with new page " << newPage << "\n";
//     // Update physical memory mapping here
//     physicalMemory.erase(oldestPage);  // Remove victim page from memory

//     for (auto& processEntry : processMap) {
//             for (auto& pageTableEntry : processEntry.second.pageTable) {
//                 if (pageTableEntry.physicalPage == oldestPage) {
//                     pageTableEntry.physicalPage = -1;  // Mark as not in physical memory
//                     pageTableEntry.isInMemory = false;  // Mark as swapped out
//                 }
//             }
...
//     physicalMemory[newPage] = 0;  // Initialize new page in physical memory
// }
